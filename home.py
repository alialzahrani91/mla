...
# ================== CONFIG ==================
st.set_page_config("AI High Gain Dashboard – TASI", layout="wide")
...
def compute_indicators(df):
    ...
    df["MACD"] = (
        df["Price"].ewm(span=12).mean() -
        df["Price"].ewm(span=26).mean()
    )

    df["ATR"] = (
        df["Price"].rolling(14).max() -
        df["Price"].rolling(14).min()
    )
...
with tab2:
    ...
    if len(training) < 50:
        ...
        winners = df_pred[df_pred["Final_Prob"] >= 0.6] \
...
with tab3:
    ...
    if preds.empty:
        ...
        merged["Target"] = (
            (merged["Change %"] >= 5) &
            (merged["Volume"] > merged["Volume"].rolling(20).mean())
        ).astype(int)
